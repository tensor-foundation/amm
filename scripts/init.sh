...
OLD_AUTHOR="author \<author\@email\.com\>"
...
find $ROOT_DIR/scripts/generate-clients.cjs -type f -print0 | xargs -0 perl -pi -e "s/$CAMEL_OLD_NAME/$CAMEL_NAME/g"
...
find $ROOT_DIR \
  ...
  s/$OLD_ORGANIZATION/$ORGANIZATION/g; \
  s/$OLD_AUTHOR/$AUTHOR/g; \
  s/$OLD_DESCRIPTION/$DESCRIPTION/g; \
  s/$OLD_PUBLIC_KEY/$PUBLIC_KEY/g"

# Update folder and file names
mv "$ROOT_DIR/program/idl/${SNAKE_OLD_NAME}_program.json" "$ROOT_DIR/program/idl/${SNAKE_NAME}_program.json"
